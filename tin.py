...
class DEF(META):
    def __call__(self, rep): 
        ...
        def execute_rec(op, ip, tokens, stack):
            self.prg.execute(stack)
...
def dup(op, ip, tokens, stack):
    stack.append(stack[-1])

    return ip, tokens, stack

def copy(op, ip, tokens, stack):
    stack[-1] = stack[-(stack[-1] + 1)]

    return ip, tokens, stack

def swap(op, ip, tokens, stack):
    stack[-1], stack[-2] = stack[-2], stack[-1]

    return ip, tokens, stack

def skip(op, ip, tokens, stack):
    if stack.pop(): 
        i += 1

branch_stack = []

def branch_init(op, ip, tokens, stack):
    branch_stack.append(ip)    

    return ip, tokens, stack

def branch_end(op, ip, tokens, stack):
...
def foreach_init(op, ip, tokens, stack):
...
def foreach_end(op, ip, tokens, stack):
...
def storer_init(op, ip, tokens, stack):
    storer_stack.append(len(stack))    

    return ip, tokens, stack

def storer_end(op, ip, tokens, stack):
...
def define_var(op, ip, tokens, stack):
    name = op[1:]
...
def delete_var(op, ip, tokens, stack):
    name = op[1:]
    variables[name].pop()
...
def get_var(op, ip, tokens, stack):
    name = op[1:]
    ...
    def execute(self, stack=None):
        ...
        while ip < len(self.tokens):
            ...
            if isinstance(tok, META):
                ip, self.tokens, stack = tok.f(tok.rep, ip, self.tokens, stack)
